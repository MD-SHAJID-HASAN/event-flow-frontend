...
function UserEvents() {
  ...
  useEffect(() => {
    const fetchUserEvents = async () => {
      ...
      try {
        const response = await axios.get(`https://eventflowbackend-1.onrender.com/api/events?userEmail=${user.email}`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this event?")) {
      try {
        await axios.delete(`https://eventflowbackend-1.onrender.com/api/events/${id}`);
